...
EXEDIR=${MAINDIR}/mpeg-pcc-tmc2-featuresExtracting/bin;
CFGDIR=${MAINDIR}/mpeg-pcc-tmc2-featuresExtracting/cfg

SEQ=27;       # in [21;28]
COND="C2AI";       # in [C2AI, C2RA, CWAI, CWRA]
FRAMECOUNT=8;
MANNER="featuresExtracting_Test";
THREAD=1;
...
do
  case $RATE in
      5) CFGRATE="rate/ctc-r5.cfg";OUTPUT=${MAINDIR}"/__output__/"${MANNER}"/"${SEQNAME}${FRAMECOUNT}_${COND}"/r5/";;
      4) CFGRATE="rate/ctc-r4.cfg";OUTPUT=${MAINDIR}"/__output__/"${MANNER}"/"${SEQNAME}${FRAMECOUNT}_${COND}"/r4/";;
      3) CFGRATE="rate/ctc-r3.cfg";OUTPUT=${MAINDIR}"/__output__/"${MANNER}"/"${SEQNAME}${FRAMECOUNT}_${COND}"/r3/";;
      2) CFGRATE="rate/ctc-r2.cfg";OUTPUT=${MAINDIR}"/__output__/"${MANNER}"/"${SEQNAME}${FRAMECOUNT}_${COND}"/r2/";;
      1) CFGRATE="rate/ctc-r1.cfg";OUTPUT=${MAINDIR}"/__output__/"${MANNER}"/"${SEQNAME}${FRAMECOUNT}_${COND}"/r1/";;
  ...
  $ENCODER \
    ...
    --normalDataPath=${SRCDIR}/$NDATAPATH | tee -a ${MAINDIR}/__statisticData__/${MANNER}${FRAMECOUNT}_${SEQNAME}_${COND}.txt
